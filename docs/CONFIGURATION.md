...
| `workflow.session_outlives_turn` | boolean | `true` | When `true` (default), executor agents are dispatched asynchronously in the background. When `false`, executor agents are dispatched synchronously in the foreground and awaited sequentially before the next executor begins (multi-plan waves run sequentially). Designed for wrapper hosts whose orchestrator session terminates with the active turn. Does not affect tool availability, runtime selection, git worktree isolation, or verifier dispatch. #3159 |

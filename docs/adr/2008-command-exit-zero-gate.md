...
| Result mapping | exit 0 → `block:false`; non-zero → `block:true`; timeout (`error.code === 'ETIMEDOUT'`, via `isSpawnTimeout`) → `block:true` (`timed_out`); `sh` missing (ENOENT, exit 127) → `block:true` | Fail-closed: every non-zero outcome blocks. A blocking gate with `block:true` halts per the existing two-step gate contract |

...
| `workflow.session_outlives_turn` | `true` | When `true` (default), executor agents are dispatched asynchronously in the background. When `false`, executor agents are dispatched synchronously in the foreground and awaited sequentially before the next executor begins (multi-plan waves run sequentially). Designed for wrapper hosts whose orchestrator session terminates with the active turn. Does not affect tool availability, runtime selection, git worktree isolation, or verifier dispatch. |
...
| `workflow.session_outlives_turn` | boolean | `true` | `true`, `false` | When `false`, dispatches executor agents in the foreground and awaits each sequentially before the next executor begins. Does not affect tool availability, runtime selection, git worktree isolation, or verifier dispatch. |

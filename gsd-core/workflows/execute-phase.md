...
- Optional `--ws NAME` → `GSD_WS="--ws NAME"` for every scoped CLI call
...
GSD_WS=""; if [[ "$ARGUMENTS" =~ (^|[[:space:]])--ws[[:space:]]+([A-Za-z0-9._-]+) ]]; then GSD_WS="--ws ${BASH_REMATCH[2]}"; fi
if [ -n "$GSD_WS" ]; then
  INIT=$(gsd_run query init.execute-phase "${PHASE_ARG}" $WAVE_PARAM --ws "${GSD_WS#--ws }")
else

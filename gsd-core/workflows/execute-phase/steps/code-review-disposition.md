...
# Built BEFORE the command, not inside its env-prefix list. A prefix assignment is visible only to
# the forked process, so the `${PADDED}` in a later prefix entry expanded the OUTER variable rather
# than the one being set two entries earlier (ShellCheck SC2097/SC2098). It happened to be the same
# value here, which is exactly why it would have gone on being wrong quietly.

...
describe('#3926 — Tier-3 scope is the phase change set, not everything since the phase began', () => {
  ...
  function runSpawnFence(repo) {
    const script = [
      'PADDED_PHASE=06',
      `PHASE_DIR=${PHASE_DIR}`,
      // spawn_reviewer reuses compute_file_scope's DIFF_BASE verbatim (#4209
      // B3), so the fence under test is composed with the shipped derivation
      // that sets it — never a hand-written stand-in.
      extractSpawnReviewerDerivation(),
      extractSpawnFence(),
      'echo "===DIFF_BASE==="',
      'echo "${DIFF_BASE}"',
      'echo "===DIFF_TIP==="',
  ...
  test(
    ...
    () => {
      ...
      // is pre-existing on this path — DIFF_BASE is the phase-directory anchor
      // compute_file_scope derives (#3191/#3503/#3995 pin it; spawn_reviewer
      // reuses it verbatim since #4209 B3), and the agent defaults an absent
      // diff_tip to HEAD for backward compatibility with direct invocations —
      // and closing it means changing the agent's default, which is a separate
      // concern. What this test pins is that the gap is ANNOUNCED rather than
      // silent, and that the anchor is not broken in passing. Raised by the
      // #3926 pre-push adversarial review.
